<x-layout>
    <div class="edit-car-listing-page container mx-auto mt-10">
        ...
        <form method="POST" action="{{ route('listings.update', $listing) }}" class="mb-10"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                {{-- car maker --}}
                <x-input-select id='car_maker' name='car_maker' label='Car Maker *'
                    value="{{ old('car_maker', $listing->car_maker) }}" disabledOptionLabel='Select car maker'
                    :options="$carMakers" :required="true" />

                {{-- price --}}
                <x-input-text id='price' name='price' type='number' label='Price *'
                    value="{{ old('price', $listing->price) }}" placeholder='Enter the price' :required="true" />
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <x-input-select id='transmission' name='transmission' label='Transmission *'
                    value="{{ old('transmission', $listing->transmission) }}"
                    :options="['Manual' => 'Manual', 'Automatic' => 'Automatic']"
                    disabledOptionLabel='Select the transmission' :required="true" />

                {{-- fuel type --}}
                <x-input-select id='fuel_type' name='fuel_type' label='Fuel type *'
                    value="{{ old('fuel_type', $listing->fuel_type) }}" disabledOptionLabel='Select fuel type'
                    :options="['Gasoline' => 'Gasoline', 'Diesel' => 'Diesel']" :required="true" />
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <x-input-select id='engine_history' name='engine_history' label='Engine History *'
                    value="{{ old('engine_history', $listing->engine_history) }}"
                    disabledOptionLabel='Select the engine history'
                ...
                <x-input-select id='engine_condition' name='engine_condition' label='Engine Condition *'
                    value="{{ old('engine_condition', $listing->engine_condition) }}"
                    disabledOptionLabel='Select the engine condition'
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <x-input-select id='restoration_history' name='restoration_history' label='Restoration History *'
                    value="{{ old('restoration_history', $listing->restoration_history) }}"
                    disabledOptionLabel='Select the restoration history'
                ...
                <x-input-text id='original_parts_percentage' name='original_parts_percentage' type='number'
                    label='Original parts percentage'
                    value="{{ old('original_parts_percentage', $listing->original_parts_percentage) }}"
                    placeholder='Original parts percentage (%)' />

                {{-- body type - null --}}
                <x-input-select id='body_type' name='body_type' label='Car Body Type'
                    value="{{ old('body_type', $listing->body_type) }}" disabledOptionLabel='Select the car body type'
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <x-input-select id='seat_material' name='seat_material' label='Seat Material'
                    value="{{ old('seat_material', $listing->seat_material) }}"
                    disabledOptionLabel='Select the seat material'
                    :options="['Leather' => 'Leather', 'Cloth' => 'Cloth', 'Vinyl' => 'Vinyl', 'Velour' => 'Velour']" />

                {{-- odometer --}}
                <x-input-select id='odometer' name='odometer' label='Odometer *'
                ...
                <x-input-select id='documentation_status' name='documentation_status' label='Documentation Status *'
                    value="{{ old('documentation_status', $listing->documentation_status) }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <x-input-text id='exterior_color' name='exterior_color' label='Exterior Color'
                    value="{{ old('exterior_color', $listing->exterior_color) }}"
                    placeholder='Enter the exterior color' />

                {{-- exterior condition - null --}}
                <x-input-select id='exterior_condition' name='exterior_condition' label='Exterior Condition'
                    value="{{ old('exterior_condition', $listing->exterior_condition) }}"
                    disabledOptionLabel='Select the exterior condition'
                ...
                <x-input-text id='interior_color' name='interior_color' label='Interior Color'
                    value="{{ old('interior_color', $listing->interior_color) }}"
                    placeholder='Enter the interior color' />

                {{-- interior condition - null --}}
                <x-input-select id='interior_condition' name='interior_condition' label='Interior Condition'
                    value="{{ old('interior_condition', $listing->interior_condition) }}"
                    disabledOptionLabel='Select the interior condition'
                    :options="['Showroom Condition' => 'Showroom Condition', 'Good' => 'Good', 'Fair' => 'Fair', 'Needs Restoration' => 'Needs Restoration']" />
            </div>

            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                {{-- city --}}
                <x-input-text id='location_city' name='location_city' label='City *'
                    value="{{ old('location_city', $listing->location_city) }}" placeholder='Enter the city name'
                    :required="true" />

                {{-- state --}}
                <x-input-text id='location_state' name='location_state' label='State *'
                    value="{{ old('location_state', $listing->location_state) }}" placeholder='Enter the state name'
                    :required="true" />

                {{-- zipcode --}}
                <x-input-text id='location_zipcode' name='location_zipcode' type='number' label='Zipcode *'
                    value="{{ old('location_zipcode', $listing->location_zipcode) }}" placeholder='Enter the zipcode'
                    :required="true" />
            </div>
            ...
            <div class="grid grid-cols-1">
                ...
                <x-input-text-area id='description' name='description' label='Car description *'
                    value="{{ old('description', $listing->description) }}" placeholder='Enter car description'
                    :required="true" />
            </div>
